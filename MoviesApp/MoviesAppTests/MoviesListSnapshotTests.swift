...
final class MoviesListSnapshotTests: XCTestCase {
    ...
    func test_moviesListWithFailedImageLoading() {
        let sut = makeSUT()
        
        sut.displayWithFailedImageLoading()
        
        assert(snapshot: sut.snapshot(for: .iPhone8(style: .light)), record: true, named: "MOVIES_LIST_WITH_FAILED_IMAGE_LOADING_light")
    }
...
extension MoviesListViewController {
    ...
    func displayWithFailedImageLoading() {
        let cellsStubs = [
            MovieCellControllerStub(
                title: "Black Panther",
                overview: "overview 1",
